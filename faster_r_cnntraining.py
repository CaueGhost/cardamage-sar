"""
fasterrcnn_train.py — Treinamento e avaliação com Faster R-CNN
...
# ── Salva o modelo ──────────────────────────────────────────────────────────────
save_path = os.path.join(DIR, "..", "fasterrcnn_best.pt")
...
for img_info in tqdm(val_data["images"], desc="Avaliando", unit="img"):
    ...
    # Painel direito — Predição Faster R-CNN
    axes[1].imshow(img_pil)
    axes[1].set_title(f"Faster R-CNN ({n_detec} detecções)", color="steelblue", fontweight="bold")
    axes[1].axis("off")
    for box, score in boxes_det:
        x1, y1, x2, y2 = box.tolist()
        axes[1].add_patch(patches.Rectangle(
            (x1, y1), x2-x1, y2-y1, linewidth=2, edgecolor="deepskyblue", facecolor="none"))
        axes[1].text(x1, y1 - 4, f"{score:.2f}", color="deepskyblue",
                     fontsize=8, fontweight="bold")

    plt.tight_layout()
    plt.savefig(
        os.path.join(OUT, f"pred_{img_info['file_name']}"),
...
# ── Resumo final ────────────────────────────────────────────────────────────────
print("\n" + "=" * 45)
print("     RESULTADO FINAL — Faster R-CNN")
print("=" * 45)
